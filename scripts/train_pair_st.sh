#!/usr/bin/env bash
# ST (selective suppression + conserved transfer): replaces the exponential tilt with
# bounded probability-mass transfer from shared-head visually-opposed donors
# D = {u<-tau} ∩ Head(p⁺;rho) ∩ Head(s;rho) to recipients {u>=-tau}, delta=min(eps,
# alpha_max·M_D), groups rescaled proportionally, tail untouched. Primary control:
# supn8 (same 48×n8 regime, u⁻ tilt). Same pair data, seed42, w_i≡1.
# 用法: bash scripts/train_pair_st.sh [hydra override...]
set -euo pipefail
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

export MODEL_PATH="${MODEL_PATH:-Qwen/Qwen3.5-4B}"
export EXPERIMENT_NAME_OVERRIDE="${EXPERIMENT_NAME_OVERRIDE:-pair_stn8_6karmA}"
...
exec "${PROJECT_ROOT}/scripts/run_visual_counterfactual_unit.sh" \
    ...
    actor_rollout_ref.actor.self_distillation.counterfactual_st_tau="${ST_TAU:-0.10}" \
