...
export EXPERIMENT_NAME_OVERRIDE="${EXPERIMENT_NAME_OVERRIDE:-pair_sup_6karmA}"
...
export COUNTERFACTUAL_NULL_MODE=mean_color COUNTERFACTUAL_EXTRAPOLATION_BETA="${SUP_BETA:-4.0}"
export ALPHA=0.5 LR=2e-6 MAX_PROMPT_LENGTH=8192 MAX_RESPONSE_LENGTH=1024 DATA_SEED=42
export TRAIN_BATCH_SIZE=48 PPO_MIMI_BATCH_SIZE=48 ROLLOUT_N=2
export TRAINER_N_GPUS_PER_NODE=2 TRAINER_NNODES=1
...
echo "${EXPERIMENT_NAME_OVERRIDE}: suppression-only u⁻ tilt β=${COUNTERFACTUAL_EXTRAPOLATION_BETA} (seed42, pair)"
exec "${PROJECT_ROOT}/scripts/run_visual_counterfactual_unit.sh" \
    ...
    actor_rollout_ref.actor.self_distillation.counterfactual_u_clip_pos=True \
