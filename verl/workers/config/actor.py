...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        ...
        teacher_regularization (str): Teacher regularization mode. Options: "frozen", "ema", "trust-region", "progressive".
        ...
        counterfactual_null_mode (Optional[str]): Deterministic visual counterfactual used to reconstruct the
            teacher target. ``"mean_color"`` replaces every teacher image with a same-size mean-color image.
        counterfactual_extrapolation_beta (float): Number of visual residual units added to the real-image
            teacher logits. ``1.0`` is the original visual-counterfactual target and ``2.0`` is strong extrapolation.
    ...
    def __post_init__(self):
        ...
        valid_teacher_regularization = ["frozen", "ema", "trust-region", "progressive"]
